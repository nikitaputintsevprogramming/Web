...
PICKUP_SHIPPING_OPTION.add(LabeledPrice('Самовывоз в Санкт-Петербурге', 1000))
...
# lambda q: True -> означает, что если все упомянутое выше было правдой
@dp.shipping_query_handler(lambda q: True)
async def shipping_process(shipping_query: ShippingQuery):
    # Если доставка невозможна в эту страну
    ...
    # Если доставка возможна в эту страну, то создаем список
    shipping_options = [SUPERSPEED_SHIPPING_OPTION]

    # Если доставка по РФ, то есть возможность оформить Почту России
    if shipping_query.shipping_address.country_code == 'RU':
        shipping_options.append(POST_SHIPPING_OPTION)

        # Если доставка еще в СПБ, то можно оформить самовывоз
...
# Если все предыдущее сработало успешно
